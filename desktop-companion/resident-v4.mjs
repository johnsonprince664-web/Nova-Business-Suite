...
async function ensureLocalSecret() {
  try {
    const current = (await fs.readFile(LOCAL_SECRET_FILE, "utf8")).trim();
    if (current) return current;
  } catch {}
  const secret = crypto.randomBytes(32).toString("base64url");
  await fs.writeFile(LOCAL_SECRET_FILE, secret, "utf8");
  return secret;
}

const localSecret = await ensureLocalSecret();
console.log(`JARVIS Resident V4 paired to ${user.email}.`);
...
function windowsStart(target) {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code) => code === 0 ? resolve() : reject(new Error(`Windows start exited ${code}`)));
  });
}
...
function lockPc() {
  return new Promise((resolve, reject) => {
    ...
    child.once("error", reject);
    child.once("exit", (code) => code === 0 ? resolve() : reject(new Error(`Lock command exited ${code}`)));
  });
}

async function executeDeviceCommand(command, args = {}) {
  if (process.platform !== "win32") throw new Error("JARVIS Resident V4 currently targets Windows.");
  if (command === "open_app") {
...
function normalizeSpeech(text) {
  return String(text || "")
    ...
    .replace(/^\s*wake\s+up\s+jarvis[\s,.:;!\-]*/i, "")
    .trim();
}
...
async function buildCrmContext() {
  ...
  const units = inventory.reduce((sum, item) => sum + num(item.qty), 0);
  const lowStock = inventory.filter((item) => num(item.qty) <= num(item.low_stock_threshold ?? 1)).length;
...
const channel = supabase
  .channel(`jarvis-resident-v4-${user.id}`)
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/health" && req.method === "GET") {
    res.writeHead(200);
    res.end(JSON.stringify({ ok: true, user: user.email, resident: true, version: 4, wakeEngine: "windows-system-speech" }));
    return;
  }
...
console.log("Local voice bridge online.");

const wakePath = path.join(__dirname, "wake-listener.ps1");
await fs.access(wakePath);
const wakeProcess = spawn("powershell.exe", [
  "-NoProfile",
...
wakeProcess.once("error", (error) => {
  console.error(`Windows wake listener failed to start: ${error.message || error}`);
  process.exitCode = 1;
});
wakeProcess.once("exit", (code) => {
  if (code !== 0) console.error(`Windows wake listener stopped with code ${code}.`);
});
