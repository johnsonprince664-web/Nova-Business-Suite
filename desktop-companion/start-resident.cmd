...
echo.
echo Starting Legacy JARVIS Resident V4 in diagnostic mode...
echo Keep this window open while testing Hey Jarvis.
echo.
node resident-v4.mjs
...
if not "%JARVIS_EXIT%"=="0" (
  ...
  echo.
  echo Last wake-listener log lines:
  powershell -NoProfile -Command "if (Test-Path $env:USERPROFILE+'\.legacy-jarvis\wake-listener.log') { Get-Content ($env:USERPROFILE+'\.legacy-jarvis\wake-listener.log') -Tail 30 }"
) else (
