...
## Dynamic item types and sale consumables

The `legacy_inventory.item_type` field now accepts any non-empty value up to 40 characters, allowing custom Inventory tabs such as Shipping to be used as item types.

`legacy_sale_consumables` stores packaging and shipping supplies used for each sale. Each record keeps the linked inventory item, quantity, item type, and unit-cost snapshot. New sale RPC overloads accept a `p_consumables` JSON array and atomically deduct supply stock. Delete triggers restore supply quantities during sale edits and deletions. RLS restricts all records to the authenticated business owner.
