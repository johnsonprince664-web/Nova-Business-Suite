...
function Sales({ data, search, setModal, openDocument, remove }) {
  ...
  const rows = data.sales.filter((sale) => {
    ...
    const matchesSearch = `${customerName(data, sale.customer_id)} ${items.map((item) => item.product_name).join(" ")} ${payments.map((payment) => `${payment.method} ${payment.amount} ${payment.notes || ""}`).join(" ")} ${sale.payment_method} ${sale.notes || ""}`.toLowerCase().includes(search.toLowerCase());
    const matchesFilter = filter === "All" || recordIsInCustomTab(data, "sale", sale.id, filter);
    return matchesSearch && matchesFilter;
  });

  return <Page title="Sales" subtitle="Record split payments and attach Venmo screenshots, cash deposit proof, or bank evidence to each sale." action="New sale" onAction={() => setModal({ type: "sale", presetTabId: customTabIdFromKey(filter) })} tabs={withCustomTabs(["All"], customTabs)} active={filter} setActive={setFilter}>
    <TableCard>
      {rows.length ? <table className="data-table w-full min-w-[1740px]"><thead><tr><th>Date</th><th>Customer</th><th>Custom tab</th><th>Items</th><th>Sold price</th><th>Payments</th><th>Shipping</th><th>Revenue</th><th>Profit</th><th>Tax proof</th><th>Notes</th><th /></tr></thead><tbody>
        {rows.map((sale) => {
          const items = data.saleItems.filter((item) => item.sale_id === sale.id);
          const payments = data.salePayments.filter((payment) => payment.sale_id === sale.id);
          const revenue = items.reduce((sum, item) => sum + item.qty * item.unit_price, 0);
          const cogs = items.reduce((sum, item) => sum + item.qty * item.unit_cost, 0);
          ...
          return <tr key={sale.id}>
            ...
            <td>{money(sale.delivery_cost)}</td>
            <td>{money(revenue)}</td>
            <td>{money(revenue - cogs - sale.delivery_cost)}</td>
...
function RecordModal({ modal, setModal, data, perform, api }) {
  ...
  useEffect(() => {
    ...
    setReceiptFile(null);
  }, [type, record?.id, modal.presetTabId, data.saleItems, data.salePayments, data.inventory, data.recordTabs]);
  ...
  return <Dialog open onOpenChange={(open) => !open && setModal(null)} title={`${record ? "Edit" : "New"} ${title}`} description="Legacy Jewelry Co. cloud CRM">
    <form onSubmit={submit}>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {type === "inventory" && <>
          <Field label="Item type"><Select value={form.item_type} onChange={(e) => update("item_type",e.target.value)}>{["Jewelry","Packaging","Other"].map((option) => <option key={option}>{option}</option>)}</Select></Field>
...
function defaults(type, record, data, presetTabId = "") {
  ...
  return {
    ...
    inventory:{item_type:"Jewelry",product:"",sku:"",color:"",supplier:"",qty:1,low_stock_threshold:1,unit_cost:0,sale_price:0,custom_tab_id:presetTabId || ""},
    sale:{customer_id:"",inventory_id:"",sold_at:today(),qty:1,unit_price:"",payments:[{method:"Cash",amount:"",notes:""}],delivery_cost:0,notes:"",custom_tab_id:presetTabId || ""},
