...
export function useCRM(user) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const [customers, inventory, sales, saleItems, saleConsumables, salePayments, orders, expenses, documents, documentFiles, documentLinks, customTabs, recordTabs] = await Promise.all([
        ...
        supabase.from("legacy_sale_consumables").select("*").eq("business_id", business.id).order("created_at", { ascending: true }),
      ...
      const results = [customers, inventory, sales, saleItems, saleConsumables, salePayments, orders, expenses, documents, documentFiles, documentLinks, customTabs, recordTabs];
      ...
      setData({
        ...
        saleConsumables: saleConsumables.data || [],
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "legacy_sale_consumables", filter: `business_id=eq.${data.business.id}` }, refresh)
    ...
    return {
      ...
      recordSale: async (values) => {
        const result = await supabase.rpc("legacy_record_sale", {
          ...
          p_consumables: values.consumables || [],
        });
        if (result.error) throw result.error;
