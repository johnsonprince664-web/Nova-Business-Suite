...
## Dynamic inventory item types

Inventory custom tabs now also become available in the **Item type** menu. For example, after creating an Inventory tab named **Shipping**, Shipping appears as an item type when adding or editing inventory. Creating inventory while a custom Inventory tab is active also selects that tab name automatically. Existing built-in types—Jewelry, Packaging, and Other—remain available.

## Actual sale cost and supply usage

Sales can now record the exact packaging and shipping supplies used, including boxes, gift bags, labels, mailers, and any other non-jewelry inventory item. The sale form:

- Shows every available supply from Inventory
- Lets you select multiple supplies and enter the quantity used
- Deducts those quantities from inventory when the sale is saved
- Restores quantities automatically when a sale is edited or deleted
- Calculates product cost, supply cost, actual item cost, delivery/shipping cost, projected profit, and margin live
- Stores a cost snapshot with the sale so historical profit does not change when a supply's current unit cost is edited later

The Sales table now shows supplies used, actual cost, and profit. Dashboard profit also includes sale-level packaging and shipping-supply costs.

Run `supabase_dynamic_types_actual_cost.sql` on a database that does not already have this upgrade. The production Legacy Jewelry Co. Supabase project was upgraded when this build was prepared.
